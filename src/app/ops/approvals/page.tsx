...
export default async function OpsApprovalsPage() {
  ...
  let q = supabase
    .from("messages")
    .select(
      "id, client_id, deal_id, persona_id, subject, body_text, status, created_at, requires_review, direction, from_address, to_addresses, drafted_by_agent, draft_confidence",
    )
  ...
  const personaIds = Array.from(
    new Set((messages ?? []).map((m) => m.persona_id).filter(Boolean)),
  ) as string[];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-[#8F8678]">
          Queued outbound drafts awaiting operator review. Approve schedules send; reject opens an
          escalation for the agent.
        </p>
      </div>
      ...
      <ul className="space-y-4">
        {(messages ?? []).map((m) => {
          ...
          const persona = m.persona_id ? pMap.get(m.persona_id) : undefined;
          const toAddr =
            Array.isArray(m.to_addresses) && m.to_addresses.length
              ? m.to_addresses.join(", ")
              : "—";
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] uppercase tracking-[0.2em] text-[#C9A84C]">
                    <Link href={`/ops/clients/${m.client_id}`} className="hover:underline">
